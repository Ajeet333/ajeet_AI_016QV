...
class player:
    ...
    def move(self,B,N,cur_x,cur_y):
        ...
        if self.step%6==0:
            self.flag+=1
            self.flag=self.flag%6

        if B[cur_x][(cur_y+1)%N]==0 and self.flag==3:
            self.up+=1
            return (0,1)

        if B[cur_x][(cur_y+N-1)%N]==0 and self.flag==1:
            self.down+=1
            return (0,-1)
        
        if B[(cur_x+1)%N][cur_y]==0 and self.flag==0:
            self.right+=1
            return (1,0)

        if B[(cur_x+N-1)%N][cur_y]==0 and self.flag==2:
            self.left+=1
            return (-1,0)
